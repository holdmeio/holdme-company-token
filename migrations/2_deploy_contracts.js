...
module.exports = function(deployer, network, accounts) {
	...
	if (network == 'development') {
		deployer.deploy(Holdme, OWNER_TESTRPC, INITIAL_SUPPLY, TOKEN_NAME, TOKEN_SYMBOL, DECIMALS).then(function(instance) {
			token=instance
			return deployer.deploy(HoldmeTokenSale, Holdme.address, OWNER_TESTRPC, startTimeSale, endTimeSale, beneficiary_testrpc, devTeam_testrpc, DECIMALS);
		...
		deployer.deploy(Holdme, ROPSTEN, INITIAL_SUPPLY, TOKEN_NAME, TOKEN_SYMBOL, DECIMALS).then(function(instance) {
    		return deployer.deploy(HoldmeTokenSale, Holdme.address, ROPSTEN, startTimeSale, endTimeSale, beneficiary, devTeam_ropsten, DECIMALS);
    		return token.transferOwnership(HoldmeTokenSale.address);
    	});
		...
		deployer.deploy(Holdme, PARITY_ROPSTEN, INITIAL_SUPPLY, TOKEN_NAME, TOKEN_SYMBOL, DECIMALS).then(function(instance) {
    		return deployer.deploy(HoldmeTokenSale, Holdme.address, PARITY_ROPSTEN, startTimeSale, endTimeSale, beneficiary, devTeam_ropsten, DECIMALS);
    		return token.transferOwnership(HoldmeTokenSale.address);
    	});
	}	
};
